...
object Problems_1_to_15_Scala {
	...
	def problem12 = {
		lazy val triNum = Stream.from(1).map(p => p*(p - 1)/2)
		def divisors(n : Int) = Stream.from(1).take(Math.sqrt(n).toInt).filter(i => n % i == 0).take(251).count(_ => true) * 2
		triNum.find(divisors(_) > 500).get
	}
		
	def problem13 = 
		Source.fromURL(classOf[Problems_1_to_15_Java].getResource("data/problem13.txt")).mkString.split("\\s+")
			.map(x => BigInt(x))
			.reduce(_ + _)
			.toString.substring(0,10).toLong
			
	def problem14 = {
		def collatzLength(n : Long, acc : Int = 0) : Int =
			if (n == 1) acc + 1 else collatzLength(if (n % 2 == 0) n/2 else 3*n + 1,acc + 1)
		(1 to 1000000).map(n => (n,collatzLength(n))).reduce((p,q) => if (p._2 > q._2) p else q)._1 
	}
	
	def problem15 = {
		def binomialCoefficient(n : Int, k : Int) : Long = {
			@tailrec
			def binomialCoefficient_tail(n : Int, k : Int, i : Int, acc : Long) : Long = {
				if (i == k) acc
				else 		binomialCoefficient_tail(n,k,i + 1,(acc*(n - i))/(i + 1))
			}
			if (k > n - k) binomialCoefficient_tail(n,n - k,0,1)
			else           binomialCoefficient_tail(n,k    ,0,1)
		}
